...
$result = $mysql->query("SELECT * FROM `purchase_status` WHERE `ready_id` = 2 ORDER BY `change_data` DESC");
...
?>
    <div class="coll">
        ...
        <div class="bt3" id="bt3">
            <form action="php/cabinet/btn/minus.php" method="POST"> <button type="submit" value="<?= $id ?>" name="btn">Отклонить</button></form>
        </div>
    </div>
<?php
    echo '</div>';
